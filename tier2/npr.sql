...
UPDATE Tier2Facilities SET rmp_id = replace(replace(id, ' ', ''), '-', '') FROM Tier2FacilityIds WHERE Tier2FacilityIDs.FacilityRecordID = Tier2Facilities.FacilityRecordID and type = 'RMP' and Id <> '';
...
CREATE OR REPLACE VIEW no_oil AS SELECT * FROM tier2facilities WHERE "naics_code" NOT LIKE '21%' AND "naics_code" NOT LIKE '22%' AND "naics_code" != '454312' AND "naics_code" NOT LIKE '48611%' AND "naics_code" NOT LIKE '3241%' AND "naics_code" != '324191';
